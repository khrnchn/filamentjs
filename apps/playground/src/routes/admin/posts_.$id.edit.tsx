...
export const Route = createFileRoute('/admin/posts_/$id/edit')({
  loader: ({ params }) => getPostForm({ data: params.id }),
  component: EditPost,
});
...
function EditPost() {
  ...
  const onSubmit = async (next: Record<string, unknown>) => {
    const result = await savePost({
      data: { id, values: next as Record<string, string | number | boolean | null> },
    });
    if (result.ok) {
